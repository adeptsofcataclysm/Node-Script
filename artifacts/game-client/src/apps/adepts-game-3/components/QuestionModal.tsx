...
function Fireworks({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const launchRocket = () => { rocketsRef.current.push({ x: W * (0.15 + Math.random() * 0.7), y: H, vy: -(7 + Math.random() * 4.5), color: FW_COLORS[Math.floor(Math.random() * FW_COLORS.length)], trail: [], exploded: false }); };
    const explode = (x: number, y: number, color: string) => { const count = 55 + Math.floor(Math.random() * 25); for (let i = 0; i < count; i++) { const angle = (Math.PI * 2 * i) / count + (Math.random() - 0.5) * 0.15; const speed = 0.45 + Math.random() * 2.1; const sparkColor = Math.random() < 0.3 ? FW_COLORS[Math.floor(Math.random() * FW_COLORS.length)] : color; sparksRef.current.push({ x, y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, color: sparkColor, alpha: 1, size: 2.5 + Math.random() * 2.5, tail: [] }); } };
    const spawnConfetti = () => { for (let i = 0; i < 3; i++) { confettiRef.current.push({ x: Math.random() * W, y: -12, vx: (Math.random() - 0.5) * 1.6, vy: 0.9 + Math.random() * 1.9, w: 8 + Math.random() * 10, h: 5 + Math.random() * 6, color: FW_COLORS[Math.floor(Math.random() * FW_COLORS.length)], rotation: Math.random() * Math.PI * 2, rotSpeed: (Math.random() - 0.5) * 0.12, alpha: 1 }); } };
    ...
    const animate = () => {
      ...
      for (const s of sparksRef.current) { s.tail.push({ x: s.x, y: s.y }); if (s.tail.length > 5) s.tail.shift(); s.x += s.vx; s.y += s.vy * 0.72; s.vy += 0.06; s.vx *= 0.985; s.alpha *= 0.982; if (!byColor.has(s.color)) byColor.set(s.color, []); byColor.get(s.color)!.push(s); }
      ...
      for (const c of confettiRef.current) { c.x += c.vx; c.y += c.vy * 0.74; c.vx += (Math.random() - 0.5) * 0.08; c.rotation += c.rotSpeed; ctx.save(); ctx.globalAlpha = c.alpha; ctx.fillStyle = c.color; ctx.translate(c.x, c.y); ctx.rotate(c.rotation); ctx.fillRect(-c.w / 2, -c.h / 2, c.w, c.h); ctx.restore(); }
      ctx.globalAlpha = 1;
    };
